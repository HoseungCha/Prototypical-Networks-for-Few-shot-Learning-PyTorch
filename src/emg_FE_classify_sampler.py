...
class EMG_FE_Classify_Sampler(object):
    ...
    def __init__(self, option, index= None, mode= None):
        '''
        ...
        self.nDomain = 5
        self.mode = mode
        self.iterations = (self.nSub-1) * (self.nSub-2) * self.nDomain  # 360번
    ...
    def __iter__(self):
        ...
        if self.mode == 'test':
            dSupport = 0
            support = get_idx_of_support(self.nFE, index, self.index_test_subject, dSupport, spc)
            query = []
            for dQuery in core.getIdxExclude_of_inputIndex(range(self.nDomain), [dSupport]):
                query.append(get_idx_of_query(self.nFE, index, self.index_test_subject, dQuery))
                # index 합치기
                yield (np.array(support).ravel().tolist() + np.array(query).ravel().tolist())
