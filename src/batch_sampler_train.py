...
class Batch_Sampler_Train(object):
    '''
    ...
    def __init__(self, option, trainSubject, index= None, mode= None):
        '''
        ...
        '''
        super(Batch_Sampler_Train, self).__init__()
        ...
        self.nDomain = 5
        self.mode = mode
        self.iterations = (self.nSub-2) * self.nDomain  # 360번
        self.trainSubject = trainSubject
    ...
    def __iter__(self):
        ...
        index = self.index
        # sTrain = self.trainSubject

        for sTrain in self.trainSubject:
            # Todo: training data
            for dSupport in range(self.nDomain): # Todo: domain 랜덤으로 두개 선택 하기나누기
                support= get_idx_of_support(self.nFE, index, sTrain, dSupport, spc)
                query  = []
                for dQuery in core.getIdxExclude_of_inputIndex(range(self.nDomain), [dSupport]):
                    query.append(get_idx_of_query(self.nFE, index, sTrain, dQuery))
                    # index 합치기
