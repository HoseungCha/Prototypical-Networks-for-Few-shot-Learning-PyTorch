...
class Batch_Sampler_Test(object):
    '''
    ...
    def __init__(self, option,testSubject, index= None, mode= None):
        '''
        ...
        '''
        super(Batch_Sampler_Test, self).__init__()
    ...
    def __iter__(self):
        ...
        index = self.index
        index_test_subject = self.testSubject

        dSupport = 0
        support = get_idx_of_support(self.nFE, index, index_test_subject, dSupport, spc)
        query = []
        for dQuery in core.getIdxExclude_of_inputIndex(range(self.nDomain), [dSupport]):
            query.append(get_idx_of_query(self.nFE, index, index_test_subject, dQuery))
            # index 합치기
